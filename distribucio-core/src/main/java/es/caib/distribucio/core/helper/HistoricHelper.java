...
import es.caib.distribucio.core.repository.HistoricAnotacioRepository;
import es.caib.distribucio.core.repository.HistoricBustiaRepository;
import es.caib.distribucio.core.repository.HistoricEstatRepository;
...
public class HistoricHelper {
	...
	private void calcularDadesAnotacionsUO(Date data, Map<Long, EntitatEntity> entitats,
    	...
    	for (Long unitatId : usuarisUo.keySet()) {
    		if (usuarisUo.containsKey(unitatId))
    			dadesAnotacions.get(unitatId).setUsuaris(new Long(usuarisUo.get(unitatId).size()));
    	}
    	usuarisUo.clear();
	    
	    // Guarda les noves dades
	    historicAnotacioRepository.save(dadesAnotacions.values());
	 ...
	 * @param entitats 
	 */	
	private void calcularDadesEstats(Date data, Map<Long, EntitatEntity> entitats,
	 ...
	 */	
	private void calcularDadesBustia(Date data, Map<Long, EntitatEntity> entitats,
			Map<Long, UnitatOrganitzativaEntity> unitats, Map<Long, Set<String>> usuarisUo,
			Map<Long, Set<String>> usuarisEntitat) {
    	...
    	for(Object[] dadesBustia : historicBustiaRepository.getBusties()) {
    		...
    		bustia.setUsuarisPermis(new Long(usuarisPermis.size()));
    		bustia.setUsuarisRol(new Long(usuarisRol.size()));
    		usuarisPermis.addAll(usuarisRol);
    		bustia.setUsuaris(new Long(usuarisPermis.size()));
	...
	private UnitatOrganitzativaEntity getUnitat(
		...
		if (unitat == null) {
			unitat = unitatOrganitzativaRepository.findOne(unitatId);
