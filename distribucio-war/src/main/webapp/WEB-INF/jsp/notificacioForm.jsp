...
<body>
	<c:choose>
		<c:when test="${empty documentNotificacioCommand.id}"><c:set var="formAction"><dis:modalUrl value="/document/${documentNotificacioCommand.documentId}/notificar"/></c:set></c:when>
		<c:otherwise><c:set var="formAction"><dis:modalUrl value="/expedient/${expedientId}/notificacio/${documentNotificacioCommand.id}"/></c:set></c:otherwise>
	</c:choose>
	<form:form action="${formAction}" method="post" cssClass="form-horizontal" commandName="documentNotificacioCommand" role="form">
		...
		<dis:inputHidden name="id"/>
		<dis:inputHidden name="documentId"/>
		<div class="tab-content">
			<div role="tabpanel" class="tab-pane active" id="dades">
				<c:choose>
					<c:when test="${empty documentNotificacioCommand.id}">
						<dis:inputSelect name="tipus" textKey="notificacio.form.camp.tipus" optionItems="${notificacioTipusEnumOptions}" optionValueAttribute="value" optionTextKeyAttribute="text" required="true"/>
					</c:when>
					<c:otherwise>
						<dis:inputHidden name="tipus"/>
					</c:otherwise>
				</c:choose>
				<dis:inputSelect name="estat" textKey="notificacio.form.camp.estat" optionItems="${notificacioEstatEnumOptions}" optionValueAttribute="value" optionTextKeyAttribute="text" required="true"/>
				<dis:inputSelect name="interessatId" textKey="notificacio.form.camp.destinatari" optionItems="${interessats}" optionValueAttribute="id" optionTextAttribute="identificador" placeholderKey="notificacio.form.camp.destinatari"/>
				<dis:inputText name="assumpte" textKey="notificacio.form.camp.concepte" required="true"/>
				<dis:inputTextarea name="observacions" textKey="notificacio.form.camp.descripcio"/>
				<dis:inputDate name="dataProgramada" textKey="notificacio.form.camp.data.programada" comment="notificacio.form.camp.data.programada.comment"/>
				<dis:inputDate name="dataCaducitat" textKey="notificacio.form.camp.data.caducitat" comment="notificacio.form.camp.data.caducitat.comment"/>
				<dis:inputNumber name="retard" textKey="notificacio.form.camp.retard" nombreDecimals="0" comment="notificacio.form.camp.retard.comment"/>
			</div>
			<div role="tabpanel" class="tab-pane" id="avisofici">
				<dis:inputText name="seuAvisTitol" textKey="notificacio.form.camp.avis.titol"/>
				<dis:inputTextarea name="seuAvisText" textKey="notificacio.form.camp.avis.text"/>
				<dis:inputTextarea name="seuAvisTextMobil" textKey="notificacio.form.camp.avis.text.mobil"/>
				<dis:inputText name="seuOficiTitol" textKey="notificacio.form.camp.ofici.titol"/>
				<dis:inputTextarea name="seuOficiText" textKey="notificacio.form.camp.ofici.text"/>
			</div>
			<div role="tabpanel" class="tab-pane" id="annexos">
				<dis:inputSelect name="annexos" textKey="notificacio.form.camp.annexos" optionItems="${annexos}" emptyOption="true" optionValueAttribute="id" optionTextAttribute="nom" placeholderKey="notificacio.form.camp.annexos"/>
			</div>
		</div>
		<c:choose>
