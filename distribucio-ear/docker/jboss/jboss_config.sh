...
JBOSS_PROPS_FILE=/home/jboss/apps/distribucio/jboss.properties
JBOSS_SYSTEM_PROPS_FILE=/home/jboss/apps/distribucio/jboss_system.properties
...
if ! grep -q "<system-properties" $CONFIG_STANDALONE_FILE; then
	...
	sed '/<\/socket-binding-group>/i <outbound-socket-binding name="smtp-distribucio" source-port="0" fixed-source-port="false"><remote-destination host="${env.JBOSS_MAIL_HOST}" port="${env.JBOSS_MAIL_PORT}"\/><\/outbound-socket-binding>' -i $CONFIG_STANDALONE_FILE
