...
//			private String csvRegulacio;
	...
	public void updateBackEnviarDelayData(
		...
		for (Long id : pendentsIdsGroupedByRegla) {
			RegistreEntity pend = registreRepository.getReferenceById(id);
			if (throwable == null) {
				...
				if (pend.getLastModifiedDate().isEmpty() || pend.getLastModifiedDate().get().isBefore(dataComunicacio.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime())) {
					pend.updateBackEstat(
							RegistreProcesEstatEnum.BACK_COMUNICADA,
							"Comunicada " + new SimpleDateFormat("dd/MM/yyyy HH:mm:ss").format(dataComunicacio));
				}
			} else {
