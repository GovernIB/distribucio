...
public class ConfigServiceImpl implements ConfigService {
	...
	public ConfigDto updateProperty(ConfigDto property) {
		ConfigEntity configEntity = configRepository.getReferenceById(property.getKey());
		configEntity.updateValue(!"null".equals(property.getValue()) ? property.getValue() : null);
		loadedPropertiesHelper.reloadProperties(configEntity.getGroupCode());
	...
	private void processPropertyValues(ConfigGroupDto cGroup) {
		for (ConfigDto config: cGroup.getConfigs()) {
			if (config.isJbossProperty()) {
				config.setValue(configHelper.getEnvironmentProperty(config.getKey(), null));
			}
		}
	...
	public Long getConfigAsLong(String key) {
		return configHelper.getAsLong(key);
	}
