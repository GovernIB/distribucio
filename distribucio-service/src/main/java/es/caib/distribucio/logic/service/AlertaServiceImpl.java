...
public class AlertaServiceImpl implements AlertaService {
	...
	public AlertaDto update(
		...
		AlertaEntity entity = alertaRepository.findById(alerta.getId()).orElse(null);
		if (entity == null ) throw new NotFoundException(alerta.getId(), AlertaEntity.class);
		ContingutEntity contingut = contingutRepository.findById(alerta.getContingutId()).orElse(null);
		entity.update(
				alerta.getText(),
				alerta.getError(),
				alerta.isLlegida());
	...
	public PaginaDto<AlertaDto> findPaginatByLlegida(
		...
		continguts.add(contingutRepository.getReferenceById(contingutId));
		if (paginacioHelper.esPaginacioActivada(paginacioParams)) {
			resposta = paginacioHelper.toPaginaDto(
