...
function default_1(client) {
    return async (request, next) => {
        ...
        if ((0, util_1.isSafeMethod)(request.method)) {
            return response;
        }
        if (!response.ok) {
        ...
        if (response.headers.has('Link')) {
            for (const httpLink of LinkHeader.parse(response.headers.get('Link')).rel('invalidates')) {
                const uri = (0, uri_1.resolve)(request.url, httpLink.uri);
                stale.push(uri);
            }
        }
        // Location headers should also expire
        if (response.headers.has('Location')) {
            stale.push((0, uri_1.resolve)(request.url, response.headers.get('Location')));
        }
        ...
        if (response.headers.has('Content-Location')) {
            const cl = (0, uri_1.resolve)(request.url, response.headers.get('Content-Location'));
            const clState = await client.getStateForResponse(cl, response);
