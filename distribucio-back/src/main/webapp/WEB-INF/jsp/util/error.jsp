...
<c:choose>
	<c:when test="${errorObject.throwable.class.canonicalName == 'es.caib.distribucio.logic.intf.exception.NotFoundException'}">
		<c:set var="errorTitol"><spring:message code="error.titol.not.found"/></c:set>
	</c:when>
	<c:when test="${errorObject.throwable.class.canonicalName == 'es.caib.distribucio.logic.intf.exception.SistemaExternException'}">
...
<body>
	<c:choose>
		<c:when test="${errorObject.throwableClassName == 'java.lang.SecurityException'}">
			<div class="alert alert-danger" style="margin-top: 20px;" role="alert"><strong>Error accedint a DISTRIBUCIÓ!</strong> ${errorObject.exceptionMessage}</div>
		</c:when>
		<c:otherwise>
