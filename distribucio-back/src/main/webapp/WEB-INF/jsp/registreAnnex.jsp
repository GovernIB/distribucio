		...
		<tr>
			<td><strong>
				<c:choose>					
					...
					<c:when test="${metaDada.key=='eni:id_tramite'}">
						<spring:message code="registre.annex.detalls.camp.metaData.idTramite"/>
					</c:when>
					<c:otherwise>
		...
		<td>
			...
			<c:if test="${not empty concsvBaseUrl}">
				<a class="btn btn-default btn-sm pull-right arxiu-download"
					<c:choose>
					    <c:when test="${annex.fitxerExtension == 'pdf' && annex.fitxerArxiuUuid != null && annex.arxiuEstat != 'ESBORRANY'}">
