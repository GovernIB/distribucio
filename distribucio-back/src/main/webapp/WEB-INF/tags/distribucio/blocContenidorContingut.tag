					...
					<li class="col-md-2 element-contingut element-draggable<c:if test="${not fill.document}"> element-droppable</c:if>" data-contenidor-id="${fill.id}">
						<div id="info-fill-${fill.id}" class="thumbnail element-noclick">
							<div class="text-center">
								<dis:blocIconaContingut contingut="${fill}" tamanyDoble="true"/> 
							</div>
							<div class="caption">
								<p class="text-center">
								...
								<dis:blocContenidorAccions id="accions-fill-${fill.id}" className="botons-accions-element" modeLlistat="false" contingut="${fill}"/>
							</div>
						</div>
						<script>
			...
			<tbody>
				<c:forEach var="fill" items="${fills}">
					<tr id="info-fill-${fill.id}" class="element-drag-drop">
						<td>
							<dis:blocIconaContingut contingut="${fill}"/>
						...
						<td>
							<dis:blocContenidorAccions className="botons-accions-element" modeLlistat="true" contingut="${fill}"/>
						</td>
					</tr>
				</c:forEach>
			</tbody>
...
<c:if test="${empty fills}">
	<h1 style="opacity: .1; text-align: center; margin-top: 1em"><dis:blocIconaContingut contingut="${fill}" tamanyEnorme="true"/></h1>
