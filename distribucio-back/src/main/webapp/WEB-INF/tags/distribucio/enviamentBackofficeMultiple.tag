	...
	<div class="panel-heading">
		<div class="row" role="button" data-toggle="collapse" data-parent="#registres-list" data-target="#registres-info" aria-expanded="true" aria-controls="registres-info">
			...
			<div class="col-sm-2">
				<div class="row">
					<div class="col-sm-8">
						<button type="button" class="btn btn-warning pull-right" id="cancelarBtn" style="visibility:hidden;" title="<spring:message code="registre.user.controller.massiva.enviament.backoffice.cancelar.title"/>">
...
// <![CDATA[
	...
	function enviaBackofficeAnotacions() {
		(async()=>{
			enviaBackofficeAnotacionsAsync();
		})();	
	}
	
	async function enviaBackofficeAnotacionsAsync() {
		...
		for (var i = 0; i < registresPendentsIds.length && !cancelar; ++i) {
			...
			var resultat = await enviaBackofficeAnotacioAsync(registresPendentsIds[i]);
			if (resultat) {
				correctes++;
...
// 		$('div.modal',window.parent.document).remove();		
	...
	function enviaBackofficeAnotacioAsync(registreId) {
